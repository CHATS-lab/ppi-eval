...
This repository contains reproducibility code for the experiments in
...
The WMT score TSVs are not required to be committed to this repository. They can
be regenerated with `mt-metrics-eval`.
...
The dataset export step requires `mt-metrics-eval`. If it is not available from
...
The script runs the pipeline in a fixed order: dataset export, basic power and
confidence-interval experiments, non-parametric power analysis, Type I error
simulations, paired-vs-unpaired variance analysis, PPSR/meta-metric analyses,
ranking-stability experiments, metric score/rank table generation, and the
annotation-saving validation.

Full reproduction can be computationally expensive. The slowest stages are the
permutation-based power analyses, ranking stability experiments, and
annotation-saving threshold searches. All outputs are written under `results/`.
...
- `results/type_i_error_metric_mean_shift_simulation`: synthetic Type I error illustrating
  the role of centering metric scores in the prediction-powered permutation test.
- `results/paired_vs_unpaired`: paired versus unpaired variance comparisons.
- `results/ppsr_discriminative_power`: discriminative power for PPSR and system-level meta-metrics.
- `results/segment_meta_metric_discriminative_power`: discriminative power for PPSR and   segment-level meta-metrics.
